...
public:
    ...
    void transfer(const uint8_t* tx,uint8_t* rx,size_t len) {
        handle_prog();
        if(len==2) { spi16(tx,rx); }
        else if(len==3) { spi24(tx,rx); }
        else if(len==4) { spi32(tx,rx); }
    }
...
private:
    void spi16(const uint8_t* tx,uint8_t* rx) {
        if(!s16.waiting) {
            ...
            if(s16.read) {
                ...
                if(rx){rx[0]=val>>8;rx[1]=val&0xFF;}
            } else {
    ...
    void spi24(const uint8_t* tx,uint8_t* rx) {
        if(!s24.waiting) {
            ...
            s24.addr = dat;
            s24.waiting=true;
            if(rx){rx[0]=0;rx[1]=0;rx[2]=0;}
        } else {
            if(s24.read) {
                uint16_t val = regs[s24.addr] & 0x3FFF;
                uint8_t c = crc8(val);
                if(rx){rx[0]=val>>8;rx[1]=val&0xFF;rx[2]=c;}
            } else {
                uint16_t val = ((tx[0]<<8)|tx[1]) & 0x3FFF;
                regs[s24.addr]=val;
    ...
    void spi32(const uint8_t* tx,uint8_t* rx) {
        if(!s32.waiting) {
            ...
            s32.addr = dat;
            s32.waiting=true;
            if(rx){rx[0]=0;rx[1]=0;rx[2]=0;rx[3]=0;}
        } else {
            if(s32.read) {
                uint16_t val = regs[s32.addr] & 0x3FFF;
                uint8_t c = crc8(val);
                if(rx){rx[0]=tx[0];rx[1]=val>>8;rx[2]=val&0xFF;rx[3]=c;}
            } else {
